...
class RuleForFluentSpec extends TransformLarsSpec {
  ...
  "The rule for w^f d a" should "contain now(T)" in {
    (DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a) flatMap (_.body)) should contain(now(T))
  }
  it should "generate only one rule" in {
    DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a) should have size (1)
  }

  it should "have head w_fl_d_a(T)" in {
    DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a).head.head.toString should include("w_fl_d_a")
  }
  it should "contain a_FLUENT" in {
    (DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a) flatMap (_.body)) should contain(a_FLUENT)
  }

  it should "contain only 2 elements in the body" in {
    DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a).head.body should have size (2)
  }
  ...
  it should "contain in body a_FLUENT(1)" in {
    (DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a_1) flatMap (_.body)) should contain(a_FLUENT(StringValue("1")))
  }
  ...
  it should "contain in body a_FLUENT(M, on)" in {
    (DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a_M_on) flatMap (_.body)) should contain(a_FLUENT(Variable("M"), StringValue("on")))
  }
  ...
  it should "contain in body a_FLUENT(M, on, bar)" in {
    (DefaultLarsToPinnedProgram.rulesForDiamond(w_fl_d_a_M_on_bar) flatMap (_.body)) should contain(a_FLUENT(Variable("M"), StringValue("on"), StringValue("bar")))
  }
