...
class RuleForDiamondSpec extends TransformLarsSpec {
  ...
  "The rule for w^2 d a" should "return two rules" in {
    LarsToPinnedProgram.rulesForDiamond(w_tu_2_d_a) should have size (2)
  }
  it should "contain now(T) in all rules" in {
    forAll(LarsToPinnedProgram.rulesForDiamond(w_tu_2_d_a)) { rule => rule.body should contain(now(T)) }
  }
  it should "have head w_tu_1_b_a" in {
    forAll(LarsToPinnedProgram.rulesForDiamond(w_tu_2_d_a)) { rule => rule.head.toString should include("w_tu_2_d_a") }
  }
  it should "contain a_TUPLE(0) for one element" in {
    forExactly(1, LarsToPinnedProgram.rulesForDiamond(w_tu_2_d_a)) { rule => rule.body should contain(a_TUPLE(0)) }
  }
  it should "contain a_TUPLE(1)" in {
    forExactly(1, LarsToPinnedProgram.rulesForDiamond(w_tu_2_d_a)) { rule => rule.body should contain(a_TUPLE(1)) }
  }

  "The rule for w^3 d a" should "contain a_TUPLE(1), a_TUPLE(2),  a_TUPLE(0)" in {
    LarsToPinnedProgram.rulesForDiamond(WindowAtom(SlidingTupleWindow(3), Diamond, a)).
