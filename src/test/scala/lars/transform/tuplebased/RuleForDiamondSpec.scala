...
class RuleForDiamondSpec extends TransformLarsSpec {
  ...
  it should "contain a(T) for one element" in {
    forExactly(1, PlainLarsToAsp.rulesForDiamond(w_te_1_d_a)) { rule => rule.body should contain(a(T)) }
  }
  it should "contain a(T - 1)" in {
    forExactly(1, PlainLarsToAsp.rulesForDiamond(w_te_1_d_a)) { rule => rule.body should contain(a(T - 1)) }
  }
