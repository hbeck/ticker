...
class RuleForBoxSpec extends TransformLarsSpec {
  ...
  "The rule for w^1 b a" should "contain now(T)" in {
    (PlainLarsToAsp.rulesForBox(w_te_1_b_a) flatMap (_.body)) should contain(now(T))
  }
  it should "have head w_te_1_b_a(T)" in {
    PlainLarsToAsp.rulesForBox(w_te_1_b_a).head.head.toString should include("w_te_1_b_a")
  }
  it should "contain a(T)" in {
    (PlainLarsToAsp.rulesForBox(w_te_1_b_a) flatMap (_.body)) should contain(a(T))
  }
  it should "contain a(T - 1)" in {
    (PlainLarsToAsp.rulesForBox(w_te_1_b_a) flatMap (_.body)) should contain(a(T - 1))
  }
