...
class RuleForBoxSpec extends TransformLarsSpec {
  ...
  "The rule for w^2 b a" should "contain now(T)" in {
    (LarsToPinnedProgram.rulesForBox(w_tu_2_b_a) flatMap (_.body)) should contain(now(T))
  }
  it should "generate only one rule" in{
    LarsToPinnedProgram.rulesForBox(w_tu_2_b_a) should have size(1)
  }
  it should "have head w_te_2_b_a(T)" in {
    LarsToPinnedProgram.rulesForBox(w_tu_2_b_a).head.head.toString should include("w_tu_2_b_a")
  }
  it should "contain a_TUPLE(0)" in {
    (LarsToPinnedProgram.rulesForBox(w_tu_2_b_a) flatMap (_.body)) should contain(a_TUPLE(0))
  }
  it should "contain a_TUPLE(1)" in {
    (LarsToPinnedProgram.rulesForBox(w_tu_2_b_a) flatMap (_.body)) should contain(a_TUPLE(1))
  }
  it should "contain only 3 elements in the body" in {
    LarsToPinnedProgram.rulesForBox(w_tu_2_b_a).head.body should have size(4)
  }

  "The rule for w^3 b a" should "contain a_TUPLE(0) a_TUPLE(1), a_TUPLE(2)" in {
    (LarsToPinnedProgram.rulesForBox(WindowAtom(SlidingTupleWindow(3), Box, a)).
