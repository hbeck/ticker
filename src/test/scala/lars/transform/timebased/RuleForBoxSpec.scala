...
class RuleForBoxSpec extends TransformLarsSpec {
  ...
  "The rule for w^1 b a" should "contain now(T)" in {
    (LarsToPinnedProgram.rulesForBox(w_te_1_b_a) flatMap (_.body)) should contain(now(T))
  }
  it should "have head w_te_1_b_a(T)" in {
    LarsToPinnedProgram.rulesForBox(w_te_1_b_a).head.head.toString should include("w_te_1_b_a")
  }
  it should "contain a(T)" in {
    (LarsToPinnedProgram.rulesForBox(w_te_1_b_a) flatMap (_.body)) should contain(a(T))
  }
  it should "contain a(T - 1)" in {
    (LarsToPinnedProgram.rulesForBox(w_te_1_b_a) flatMap (_.body)) should contain(a(T - 1))
  }

  "The rule for w^3 b a" should "contain a(T) a(T -1), a(T -2), a(T -3)" in {
    (LarsToPinnedProgram.rulesForBox(WindowAtom(SlidingTimeWindow(3), Box, a)) flatMap (_.body)) should contain allOf(a(T), a(T - 1), a(T - 2), a(T - 3))
  }
  ...
  "The rule for w^1 b a(1)" should "have head w_te_1_b_a(1, T)" in {
   val head = LarsToPinnedProgram.rulesForBox(w_te_1_b_a_1).head.head
