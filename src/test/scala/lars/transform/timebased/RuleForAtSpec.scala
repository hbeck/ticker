...
class RuleForAtSpec extends TransformLarsSpec {
  ...
  "The rule for w^1 at_1 a" should "return two rules" in {
    PlainLarsToAsp.rulesForAt(w_te_1_a_1_a) should have size (2)
  }
  ...
  it should "contain now(t0)" in {
    forExactly(1, PlainLarsToAsp.rulesForAt(w_te_1_a_1_a)) { rule => rule.body should contain(now(t0)) }
  }
  it should "contain now(t1)" in {
    forExactly(1, PlainLarsToAsp.rulesForAt(w_te_1_a_1_a)) { rule => rule.body should contain(now(t1)) }
  }
  it should "have head w_te_1_at_1_a" in {
    forAll(PlainLarsToAsp.rulesForAt(w_te_1_a_1_a)) { rule => rule.head.toString should include("w_te_1_at_1_a") }
  }
  it should "contain a(t1) for all elements" in {
    forAll(PlainLarsToAsp.rulesForAt(w_te_1_a_1_a)) { rule => rule.body should contain(a(t1)) }
  }
  ...
  "The window atom wˆ at_U a" should "have one rule with head w_te_1_at_U_a" in {
    forExactly(1, PlainLarsToAsp.rulesForAt(w_te_1_at_U_a)) { rule => rule.head.toString should startWith("w_te_1_at_U_a") }
  }
