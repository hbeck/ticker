...
class RuleForAtTimeVariable extends TransformLarsSpec {
  ...
  "The rule for w^1 at_1 a" should "return 3 rules" in {
    DefaultLarsToPinnedProgram.rulesForAtTimeVariable(w_te_1_at_U_a, U) should have size (2)
  }
  ...
  it should "return two rules with head w_1_at_1_a" in {
    forExactly(2, DefaultLarsToPinnedProgram.rulesForAtTimeVariable(w_te_1_at_U_a, U)) {
  ...
  it should "contain a(T) for one element" in {
    forExactly(1, DefaultLarsToPinnedProgram.rulesForAtTimeVariable(w_te_1_at_U_a, U)) { rule => rule.body should contain(a(T)) }
  }
