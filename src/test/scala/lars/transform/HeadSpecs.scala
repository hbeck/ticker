...
class HeadSpecs extends TransformLarsSpec {
  ...
  "The head for wˆ1 d a" should "be the Atom (w_te_1_d_a)" in {
    PlainLarsToAsp.head(w_te_1_d_a).toString should include("w_te_1_d_a")
  }
  it should "have arity 1" in {
    PlainLarsToAsp.head(w_te_1_d_a).arity should be(1)
  }
  it should "have TimeVariable T as argument" in {
    PlainLarsToAsp.head(w_te_1_d_a).toString should include("T")
  }

  "The head for wˆ1 b a" should "be w_te_1_b_a(T)" in {
    PlainLarsToAsp.head(WindowAtom(st1, Box, a)) should equal(Atom("w_te_1_b_a")(T))
  }
}
