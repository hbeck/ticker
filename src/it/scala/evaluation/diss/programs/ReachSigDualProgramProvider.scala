...
//'Dual': not before window atom (for box cases)
trait ReachSigDualProgramProvider extends AnalyticProgramProvider with Scalable {
  ...
  def program(): LarsProgram = {
    ...
    LarsProgram.from(
      reachXY <= edgeXY not wSigXY and Lt(X,Y),
