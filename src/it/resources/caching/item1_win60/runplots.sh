...
do
    ...
    java -cp $classpath $program inst cachingStatTupleNonDet pre $pre runs $runs tp $tp winsize $winSize items $items impl Doyle >> $tmsFile
done
...
do
    ...
    java -cp $classpath $program inst cachingStatTimeNonDet pre $pre runs $runs tp $tp winsize $winSize items $items impl Doyle >> $tmsFile
done
...
do
    ...
    java -cp $classpath $program inst cachingStatTupleNonDet pre $pre runs $runs tp $tp winsize $winSize items $items impl ClingoPush >> $clingoFile
done
...
do
    ...
    java -cp $classpath $program inst cachingStatTimeNonDet pre $pre runs $runs tp $tp winsize $winSize items $items impl ClingoPush >> $clingoFile
done
